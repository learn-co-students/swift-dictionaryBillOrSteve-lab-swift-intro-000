...
class ViewController: UIViewController {
    ...
    var factsForBillAndSteve: [String: [String]] = [:]
    ...
    func createFacts() {
        ...
        factsForBillAndSteve["Bill Gates"] = billFacts
        factsForBillAndSteve["Steve Jobs"] = steveFacts
    }
    ...
    func getRandomFact() -> (String, String) {
        ...
        let newRandomPerson = randomPerson()
        
        if let facts = factsForBillAndSteve[newRandomPerson] {
            let fact = produceFacts(from: facts, with: newRandomPerson)
            return (newRandomPerson, fact)
        }
        
        let otherPerson = newRandomPerson == "Bill Gates" ? "Steve Jobs" : "Bill Gates"
        
        if let facts = factsForBillAndSteve[otherPerson] {
            let fact = produceFacts(from: facts, with:otherPerson )
    ...
    func produceFacts(from array: [String],with person: String) -> String {
        var copy = array
        let index = randomIndex(fromArray: copy)
        let fact = copy[index]
        copy.remove(at: index)
        factsForBillAndSteve[person] = copy.isEmpty ? nil : copy
        return fact
    }
