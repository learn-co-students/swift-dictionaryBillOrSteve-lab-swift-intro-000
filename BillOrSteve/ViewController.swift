...
class ViewController: UIViewController {
    ...
    @IBAction func billClicked(_sender: AnyObject) {
        if correctPerson == "Bill Gates" {
            if let currentCount = scoreCounter.text{
                scoreCounter.text = String(Int(currentCount)!+1)
            }
        }
    }
    
    @IBAction func steveClicked(_sender: AnyObject) {
