...
class ViewController: UIViewController {
    ...
    @IBAction func checkSteve(_ sender: Any) {
    ...
    @IBAction func checkBill(_ sender: Any) {
    ...
    func getRandomFact() -> (String, String) {
        ...
        if let facts = billAndSteveFacts[person] {
            let fact = fabricateFact(from: facts, with: person)
            return (person, fact)
        }
    ...
    func fabricateFact(from array: [String], with person: String) -> String {
        var tempArray = array
        let index = randomIndex(fromArray: tempArray)
        let fact = tempArray[index]
        tempArray.remove(at: index)
        if tempArray.isEmpty {
            billAndSteveFacts[person] = nil
        } else {
            billAndSteveFacts[person] = tempArray
        }
        return fact
    }
