...
class ViewController: UIViewController {
    ...
    @IBAction func steveGuess(_ sender: AnyObject) {
    ...
    @IBAction func billGuess(_ sender: AnyObject) {
        if correctPerson == "Steve Jobs" {
            scoreCounter.text = String(score)
            score += 1
            showFact()
    ...
    func getRandomFact() -> (String, String) {
        ...
        if let randomPersonFact = billAndSteveFacts[person] {
            let randomFact = randomIndex(fromArray: randomPersonFact)
            personFact = randomPersonFact[randomFact]
        }
        
        return (person, personFact)
    }
    
    func showFact() {
        let personAndPersonFact = getRandomFact()
        factLabel.text = personAndPersonFact.1
        
        correctPerson = personAndPersonFact.0
