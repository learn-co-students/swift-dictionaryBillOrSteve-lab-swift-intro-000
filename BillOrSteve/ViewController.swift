...
class ViewController: UIViewController {
    ...
    var factsBillSteve: [String : [String]] = [:]
    ...
    @IBAction func billGuess(_ sender: Any) {
        if correctPerson == "Bill Gates" {
            counter += 1
            scoreCount.text = String(counter)
        }
        showFact()
    }
    ...
    func createFacts() {
        ...
        factsBillSteve["Bill Gates"] = billFacts
        factsBillSteve["Steve Jobs"] = steveFacts
    }
    
    func getRandomFact() -> (String, String) {
        let person = randomPerson()
        if let facts = factsBillSteve[person] {
            let fact = productFact(from: facts, with: person)
            return(person, fact)
        }
        
        let otherPerson = person == "Bill Gates" ? "Steve Jobs" : "Bill Gates"
        
        if let facts = factsBillSteve[otherPerson] {
            let fact = productFact(from: facts, with: otherPerson)
    ...
    func productFact(from array: [String], with person: String) -> String {
        var copy = array
        let index = randomIndex(fromArray: copy)
        let fact = copy[index]
        copy.remove(at: index)
        factsBillSteve[person] = copy.isEmpty ? nil : copy
        return fact
    }
